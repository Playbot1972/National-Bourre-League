...
describe("Pagat pot / settlement compliance", () => {
  ...
  it("10 — single bourré player pays full settled pot at next deal", () => {
    ...
    assert.equal(result.deal.collected.nextHandPot, settledPot + ante * 3);
  });

  it("11 — multiple bourré players each pay full settled pot at next deal", () => {
  ...
  it("14 — next-hand pot seeds from bourré funding plus normal antes", () => {
    const scoreById = freshScores(four);
    ...
    assert.equal(deal.collected.postedAntes.p3, 4);
    ...
    assert.equal(deal.collected.nextHandPot, 4 + ante * 3);
  });
  ...
  it("15 — integration: persisted settlement → next deal funding (production path)", () => {
    ...
    assert.equal(fresh.deal.collected.postedAntes.p4, 4);
    assert.equal(stale.settlement.carryOverPot, fresh.settlement.carryOverPot);
    assert.equal(stale.settlement.carryOverPot, 4);
