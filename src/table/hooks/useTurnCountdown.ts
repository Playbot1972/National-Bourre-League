 ...
 * from bot-play-delay so the ring matches the submit gate.
 */
export function useTurnCountdown(input: TurnCountdownInput): UseTurnCountdownResult {
  ...
  const botRingReport =
    isBotPlayTurn && activeActorId && liveTurnPlayerId === activeActorId
      ? {
  ...
  useEffect(() => {
    ...
    const nextIdentity: BotTurnIdentity | null =
      botRingReport && liveTurnPlayerId
        ...
        : null;
    if (prevIdentity && nextIdentity && isDurableBotTurnExit(prevIdentity, nextIdentity)) {
      reportVisibleBotRingHidden({
  ...
  }, [botRingReport?.turnKey, input.session.handNumber, liveTurnPlayerId, stableTrickNumber]);
