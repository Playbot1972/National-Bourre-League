...
//#endregion
//#region src/session/transitionLock.ts
function de() {
	let e = null;
...
//#endregion
//#region src/session/handTransitionLog.ts
var fe = {
...
}, pe = "nbl-game-flow-debug";
function me() {
	if (typeof globalThis > "u") return !1;
	try {
		if (globalThis.localStorage?.getItem(pe) === "1") return !0;
		let e = globalThis.location;
		if (e?.search) return new URLSearchParams(e.search).has("gameFlowDebug");
	} catch {}
	return !1;
}
function he(e, t, { force: n = !1 } = {}) {
	if (!n && !me()) return;
	let r = typeof performance < "u" ? `${performance.now().toFixed(1)}ms` : "";
	console.info(`[HAND-TRANSITION ${r}] ${e}`, t ?? "");
}
function Z(e, t) {
	console.info("[HAND-TRANSITION]", e, JSON.stringify(t ?? {}));
}
//#endregion
