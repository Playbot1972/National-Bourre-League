...
Repo-specific steps for validating open PRs before merge. **Nothing beyond [PR #65](https://github.com/Playbot1972/National-Bourre-League/pull/65) (play engine) is on `main` yet.**
...
| `npm run test:game` | Card uniqueness + draw flow tests | **#67+** (not on `main` yet) |
| `npm run test:feedback` | Haptics fallback + prefs tests | **#68+** (not on `main` yet) |
| `npm run icons:generate` | SVG → PNG icon export | **#70+** (not on `main` yet) |

`main` today: lint, build, build:game, build:table, check:social, emulators, social — **no** `test:game`, `test:feedback`, or `icons:generate`.
