...
export function antePresentationDedupeKey(
  sessionId: string,
  handNumber: number,
  playerIds: readonly string[],
  anteAmount: number,
): string {
  return `${sessionId}:${handNumber}:${playerIds.join(",")}:${anteAmount}`;
}
 ...
 * Returns false when this presentation key already ran (re-render guard).
 */
