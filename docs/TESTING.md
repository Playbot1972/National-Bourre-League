...
| GitHub Actions: Hosting deployed but Functions still old | Legacy workflow deployed Hosting first, or gcOrphan recovery skipped full deploy | Re-run **Deploy to Firebase** (Functions-first; full `functions,firestore:rules` always runs before Hosting) or **Deploy Functions Only** |
