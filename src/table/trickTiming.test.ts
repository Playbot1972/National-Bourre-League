...
describe("trickTiming", () => {
  ...
  it("uses longer read when trump beats led suit", () => {
    ...
    assert.equal(postTrickReadMs({ trumpBeat: true }), 625);
  ...
  it("schedules winner reveal inside the read pause", () => {
    ...
    assert.ok(schedule.winnerRevealMs <= WINNER_REVEAL_MS);
    assert.equal(schedule.readBeforeWinnerMs + schedule.winnerRevealMs, schedule.readTotalMs);
  });

  it("restores turn indicator after read, during collection", () => {
